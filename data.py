...
class DIV2K:
    ...
    def dataset(self, batch_size=16, repeat_count=None, random_transform=True):
        ...
        if random_transform:
            ds = ds.map(lambda lr, hr: random_crop(lr, hr, scale=self.scale, hr_crop_size=96), num_parallel_calls=AUTOTUNE)
            #ds = ds.map(random_rotate, num_parallel_calls=AUTOTUNE)
            #ds = ds.map(random_flip, num_parallel_calls=AUTOTUNE)
            #ds = ds.map(random_jpeg, num_parallel_calls=AUTOTUNE)
            #ds = ds.map(random_saturation, num_parallel_calls=AUTOTUNE)
            #ds = ds.map(random_flip_up_down, num_parallel_calls=AUTOTUNE)
...
def random_flip_up_down(lr_img, hr_img):
    rn = tf.random.uniform(shape=(), maxval=1)
    return tf.cond(rn < 0.25,
                   lambda: (lr_img, hr_img),
                   lambda: (tf.image.random_flip_up_down(lr_img),
                            tf.image.random_flip_up_down(hr_img)))

def random_jpeg(lr_img, hr_img):
    return tf.image.random_jpeg_quality(lr_img, 60, 100, seed=42), hr_img

def random_adjust_brightness(lr_img, hr_img):
    rn = tf.random.uniform(shape=(), maxval=1, dtype=tf.float32)
    return tf.image.adjust_brightness(lr_img, rn), tf.image.adjust_brightness(hr_img, rn)

def random_adjust_contrast(lr_img, hr_img):
    rn = tf.random.uniform(shape=(), minval=0.1, maxval=0.9, dtype=tf.float32)
    return tf.image.adjust_contrast(lr_img, rn), tf.image.adjust_contrast(hr_img, rn)

def random_adjust_hue(lr_img, hr_img):
    rn = tf.random.uniform(shape=(), minval=-1, maxval=1, dtype=tf.float32)
    return tf.image.adjust_hue(lr_img, rn), tf.image.adjust_hue(hr_img, rn)

def random_saturation(lr_img, hr_img):
    return tf.image.random_saturation(lr_img, 0.25, 0.5, seed=42), hr_img
